        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h4 className="font-medium text-foreground">Kampanya & Promosyon Yönetimi</h4>
              <p className="text-sm text-muted-foreground">promotions use case için gerekli</p>
            </div>
          </div>
        ...
        <div className={showCollapsible ? "p-4" : ""}>
          ...
          <div className="flex gap-0 border-b border-border mb-6">
            ...
            >
              ...
              Kampanyalar
              {!campaignsLoading && <span className="ml-1 text-xs bg-muted rounded-full px-1.5 py-0.5">{campaigns.length}</span>}
            </button>
            <button
            ...
            >
              ...
              Promosyon Kodları
              {!promoCodesLoading && <span className="ml-1 text-xs bg-muted rounded-full px-1.5 py-0.5">{promoCodes.length}</span>}
            </button>
          </div>
          ...
          {activeSection === 'campaigns' && (
            <div>
              ...
              {/* Campaign Form */}
              {showCampaignForm && (
                <div className="mb-4 p-4 border border-amber-200 dark:border-amber-800/50 bg-amber-50 dark:bg-amber-950/30 rounded-lg">
                  <div className="flex items-center justify-between mb-4">
                    <h5 className="font-medium text-foreground">
                      {editingCampaign ? 'Kampanyayı Düzenle' : 'Yeni Kampanya Ekle'}
                    </h5>
                    <button onClick={resetCampaignForm} className="text-muted-foreground hover:text-foreground">
                      <X className="w-5 h-5" />
                    </button>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    />
                    <div>
                      <label className="block text-sm font-medium text-muted-foreground mb-2">İndirim Tipi</label>
                      <select
              ...
              {!campaignsLoading && !campaignsError && campaigns.length > 0 && (
                <div className="space-y-3">
                  {campaigns.map((campaign) => (
                    <div key={campaign.id} className="border border-border rounded-xl p-4 hover:border-amber-200 dark:hover:border-amber-800/50 transition-colors">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex flex-col items-end gap-2 shrink-0">
                          ...
                          <div className="flex gap-1">
                            <button onClick={() => handleEditCampaign(campaign)} className="p-1.5 hover:bg-muted rounded-lg transition-colors">
                              <Edit3 className="w-3.5 h-3.5 text-muted-foreground" />
                            </button>
                            <button onClick={() => { setItemToDelete(campaign); setDeleteType('campaign'); setDeleteModal(true); }} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-lg transition-colors">
              ...
              {showPromoForm && (
                <div className="mb-4 p-4 border border-amber-200 dark:border-amber-800/50 bg-amber-50 dark:bg-amber-950/30 rounded-lg">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={resetPromoForm} className="text-muted-foreground hover:text-foreground">
                      <X className="w-5 h-5" />
                    </button>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              {!promoCodesLoading && !promoCodesError && promoCodes.length > 0 && (
                <div className="space-y-3">
                  {promoCodes.map((promo) => (
                    <div key={promo.id} className="border border-border rounded-xl p-4 hover:border-amber-200 dark:hover:border-amber-800/50 transition-colors">
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                          {/* Code pill */}
                          <div className="px-3 py-2 rounded-lg bg-muted font-mono text-sm font-semibold text-foreground tracking-wider border border-border">
                            {promo.code}
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="font-medium text-foreground">{formatDiscount(promo.discount_type, promo.discount_value)}</span>
                              {promo.max_uses && (
                                <span className="text-xs text-muted-foreground">• {promo.used_count || 0}/{promo.max_uses} kullanım</span>
                              )}
                            </div>
                            {promo.expires_at && (
                              <p className="text-xs text-muted-foreground mt-0.5">
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          ...
                          <div className="flex gap-1">
                            <button onClick={() => handleEditPromo(promo)} className="p-1.5 hover:bg-muted rounded-lg transition-colors">
                              <Edit3 className="w-3.5 h-3.5 text-muted-foreground" />
                            </button>
                            <button onClick={() => { setItemToDelete(promo); setDeleteType('promo'); setDeleteModal(true); }} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-lg transition-colors">
                              <Trash2 className="w-3.5 h-3.5 text-red-500" />
                            </button>
                          </div>
                        </div>
